...
/*	$(".HeroSlider_wrapper").slick({
	...
	$('form#rsvp-event').on('submit',function(e){
		...
		if(isValid){
			...
			var _data = {"name":rsvp_name, "guests":rsvp_guests, "email":rsvp_email, "title":couple_name,"nid":cardID,"wed_date":wed_date,"n_persons":rsvp_npersons};
...
if(isMobile && $('#ios_audio_lockscreen').length>0){
	$('#ios_audio_lockscreen').removeClass('hidden');
